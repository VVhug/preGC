...
int uninterrupt_gc(struct ssd_info *ssd,unsigned int channel,unsigned int chip,unsigned int die,unsigned int plane)       
{
	...
	{		
		...
		{	
			fprintf(ssd->outputfile, "当前时间:%lld,开始搬运有效页channel=%d,chip=%d,die=%d,plane=%d,block=%d,page=%d\n", ssd->current_time, channel, chip, die, plane, block, i);
			fflush(ssd->outputfile);
			location=(struct local * )malloc(sizeof(struct local ));
			alloc_assert(location,"location");
			memset(location,0, sizeof(struct local));
	...
	fprintf(ssd->outputfile, "本次不可中断GC搬运有效页个数为:%d\n", page_move_count);
	fflush(ssd->outputfile);
	...
	***************************************************************/
	...
	int tR = ssd->parameter->time_characteristics.tR * (1 + LAYER_SPPED * i / TOTAL_LAYER);
	int tPROG = ssd->parameter->time_characteristics.tPROG * (1 + LAYER_SPPED * i / TOTAL_LAYER);
	if ((ssd->parameter->advanced_commands&AD_COPYBACK)==AD_COPYBACK)
	{
		if (ssd->parameter->greed_CB_ad==1)
		{

			ssd->channel_head[channel].next_state_predict_time=ssd->current_time+page_move_count*(7*ssd->parameter->time_characteristics.tWC+tR+7*ssd->parameter->time_characteristics.tWC+tPROG);			
			ssd->channel_head[channel].chip_head[chip].next_state_predict_time=ssd->channel_head[channel].next_state_predict_time+ssd->parameter->time_characteristics.tBERS;
		} 
	} 
	else
	{
		...
		ssd->channel_head[channel].next_state_predict_time=ssd->current_time+page_move_count*(7*ssd->parameter->time_characteristics.tWC+tR+7*ssd->parameter->time_characteristics.tWC+tPROG)+transfer_size*SECTOR*(ssd->parameter->time_characteristics.tWC+ssd->parameter->time_characteristics.tRC);					
		ssd->channel_head[channel].chip_head[chip].next_state_predict_time=ssd->channel_head[channel].next_state_predict_time+ssd->parameter->time_characteristics.tBERS;
...
int interrupt_partial_gc(struct ssd_info *ssd, struct partial_gc_operation *partial_gc_node)
{
	int MAX_PAGE_MOVE_COUNT = ssd->parameter->time_characteristics.tBERS / ssd->parameter->time_characteristics.tPROG;
	unsigned int i, active_block, transfer_size, invalid_page = 0;
	struct local *location;
	struct gc_operation *gc_node = partial_gc_node->gc_node;
	unsigned int channel = partial_gc_node->channel;
	unsigned int chip = gc_node->chip;
	unsigned int die = gc_node->die;
	unsigned int plane = gc_node->plane;
	unsigned int block = gc_node->block;
	...
	int page_move_spend_time = 0;
	if (partial_gc_node->page_move_finished == 0)
	{
		if (ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[gc_node->block].invalid_page_num != ssd->parameter->page_block)     /*还需要执行copyback操作*/
		{
			for (i = gc_node->page; i<ssd->parameter->page_block; i++)
			{
				if (ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[gc_node->block].page_head[i].valid_state>0)
				{
					...
					if ((ssd->parameter->advanced_commands&AD_COPYBACK) == AD_COPYBACK)
					{
						page_move_spend_time = 7 * ssd->parameter->time_characteristics.tWC + ssd->parameter->time_characteristics.tR + 7 * ssd->parameter->time_characteristics.tWC;
						ssd->channel_head[channel].next_state_predict_time = ssd->current_time + page_move_spend_time;
						ssd->channel_head[channel].chip_head[chip].next_state_predict_time = ssd->channel_head[channel].next_state_predict_time + ssd->parameter->time_characteristics.tPROG;
					}
					else
					{
						page_move_spend_time = (7 + transfer_size*SECTOR)*ssd->parameter->time_characteristics.tWC + ssd->parameter->time_characteristics.tR + (7 + transfer_size*SECTOR)*ssd->parameter->time_characteristics.tWC;
						ssd->channel_head[channel].next_state_predict_time = ssd->current_time + page_move_spend_time;
						ssd->channel_head[channel].chip_head[chip].next_state_predict_time = ssd->channel_head[channel].next_state_predict_time + ssd->parameter->time_characteristics.tPROG;
					}
					if (page_move_count >= MAX_PAGE_MOVE_COUNT)
					{
						fprintf(ssd->outputfile, "本次不可中断GC搬运块耗时：%d，擦除块耗时：%d，开始时间：%lld，占用channel：%d，占用chip：%d\n", page_move_spend_time, 0, ssd->current_time, channel, chip);
						fflush(ssd->outputfile);
						return 0;
					}
				}
	...
	if (partial_gc_node->page_move_finished == 1 || page_move_count == 0)
	{
		erase_operation(ssd, channel, chip, die, plane, gc_node->block);
		fprintf(ssd->outputfile, "本次不可中断GC搬运块耗时：%d，擦除块耗时：%d，开始时间：%lld，占用channel：%d，占用chip：%d\n", page_move_spend_time, ssd->parameter->time_characteristics.tBERS, ssd->current_time, channel, chip);
		fflush(ssd->outputfile);
		ssd->channel_head[channel].current_state = CHANNEL_C_A_TRANSFER;
		ssd->channel_head[channel].current_time = ssd->current_time;
		ssd->channel_head[channel].next_state = CHANNEL_IDLE;
	...
	fprintf(ssd->outputfile, "本次不可中断GC搬运块耗时：%d，擦除块耗时：%d，开始时间：%lld，占用channel：%d，占用chip：%d\n", page_move_spend_time, 0, ssd->current_time, channel, chip);
	fflush(ssd->outputfile);
	partial_gc_node->page_move_finished = 1;
	return 0;
}
...
Status gc_for_channel(struct ssd_info *ssd, unsigned int channel)
{
	...
	********************************************************************************************/
	...
	struct partial_gc_operation *partial_gc_node = ssd->channel_head[channel].partial_gc_node;
	if (partial_gc_node == NULL)
	{
		struct partial_gc_operation *tmp = (struct partial_gc_operation *)malloc(sizeof(struct partial_gc_operation));
		alloc_assert(tmp, "partial_gc_operation");
		memset(tmp, 0, sizeof(struct partial_gc_operation));
		partial_gc_node = tmp;
	}
	if (partial_gc_node->next_node == NULL)		//上次gc已完成
	{
		while (gc_node != NULL)   //gc队列不为空
		{
	...
	else
	{
			...
			//if (1==1)		
		{
			...
			if (flag_direct_erase != SUCCESS)
			{
				...
				flag_gc = interrupt_partial_gc(ssd, partial_gc_node->next_node);
				if (flag_gc == 1)
				{
					partial_gc_node->next_node = partial_gc_node->next_node->next_node;
					delete_gc_node(ssd, channel, gc_node);
				}
			}
