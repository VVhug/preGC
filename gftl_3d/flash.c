...
Status copy_back(struct ssd_info * ssd, unsigned int channel, unsigned int chip, unsigned int die, struct sub_request * sub)
{
	...
	long long time = 0;
	//rui alter 根据不同的所在页计算不同的介质读写时间
	int layer = sub->location->page / (ssd->parameter->page_block / TOTAL_LAYER);
	int tR = ssd->parameter->time_characteristics.tR * (1 + LAYER_SPPED * layer / TOTAL_LAYER);
	int tPROG = ssd->parameter->time_characteristics.tPROG * (1 + LAYER_SPPED * layer / TOTAL_LAYER);
	if (ssd->parameter->greed_CB_ad == 1)                                               /*允许贪婪使用copyback高级命令*/
	{
		...
		if (ssd->dram->map->map_entry[sub->lpn].state != 0)                             /*说明这个逻辑页之前有写过，需要使用copyback+random input命令，否则直接写下去即可*/
		{
			...
			{
				sub->next_state_predict_time = ssd->current_time + 19 * ssd->parameter->time_characteristics.tWC + tR + (sub->size*ssd->parameter->subpage_capacity)*ssd->parameter->time_characteristics.tWC;
				ssd->copy_back_count++;
				ssd->read_count++;
				ssd->update_read_count++;
	...
	else                                                                              /*不能贪婪的使用copyback高级命令*/
	{
		if (ssd->dram->map->map_entry[sub->lpn].state != 0)
		{
			...
			else
			{
				...
				//rui alter 在get_ppn方法中修改了sub的page位置根据不同的所在页计算不同的介质读写时间
				layer = sub->location->page / (ssd->parameter->page_block / TOTAL_LAYER);
				tR = ssd->parameter->time_characteristics.tR * (1 + LAYER_SPPED * layer / TOTAL_LAYER);
				tPROG = ssd->parameter->time_characteristics.tPROG * (1 + LAYER_SPPED * layer / TOTAL_LAYER);
				...
				{
					sub->next_state_predict_time = ssd->current_time + 7 * ssd->parameter->time_characteristics.tWC + tR + (size(ssd->dram->map->map_entry[sub->lpn].state))*ssd->parameter->time_characteristics.tRC + (sub->size*ssd->parameter->subpage_capacity)*ssd->parameter->time_characteristics.tWC;
				}
				ssd->read_count++;
				ssd->update_read_count++;
			}
...
Status static_write(struct ssd_info * ssd, unsigned int channel, unsigned int chip, unsigned int die, struct sub_request * sub)
{
	...
	if (ssd->dram->map->map_entry[sub->lpn].state != 0)                                    /*说明这个逻辑页之前有写过，需要使用先读出来，再写下去，否则直接写下去即可*/
	{
		...
		{
			sub->next_state_predict_time = ssd->current_time + 7 * ssd->parameter->time_characteristics.tWC + tR + (size((ssd->dram->map->map_entry[sub->lpn].state^sub->state)))*ssd->parameter->time_characteristics.tRC + (sub->size*ssd->parameter->subpage_capacity)*ssd->parameter->time_characteristics.tWC;
			ssd->read_count++;
			ssd->update_read_count++;
		}
	}
	...
	//rui alter 在get_ppn方法中确定了sub的page，根据不同的所在页计算不同的介质读写时间
	layer = sub->location->page / (ssd->parameter->page_block / TOTAL_LAYER);
	tR = ssd->parameter->time_characteristics.tR * (1 + LAYER_SPPED * layer / TOTAL_LAYER);
	tPROG = ssd->parameter->time_characteristics.tPROG * (1 + LAYER_SPPED * layer / TOTAL_LAYER);
	/****************************************************************
	...
	*****************************************************************/
	...
	ssd->channel_head[channel].chip_head[chip].current_time = ssd->current_time;
	ssd->channel_head[channel].chip_head[chip].next_state = CHIP_IDLE;
	ssd->channel_head[channel].chip_head[chip].next_state_predict_time = time + tPROG;
...
struct ssd_info *compute_serve_time(struct ssd_info *ssd,unsigned int channel,unsigned int chip,unsigned int die,struct sub_request **subs, unsigned int subs_count,unsigned int command)
{
	...
	else if(command==NORMAL)
	{
		...
		//rui alter 根据不同的所在页计算不同的介质读写时间
		int layer = subs[0]->location->page / (ssd->parameter->page_block / TOTAL_LAYER);
		int tR = ssd->parameter->time_characteristics.tR * (1 + LAYER_SPPED * layer / TOTAL_LAYER);
		int tPROG = ssd->parameter->time_characteristics.tPROG * (1 + LAYER_SPPED * layer / TOTAL_LAYER);
		...
		ssd->channel_head[channel].chip_head[chip].current_time=ssd->current_time;									
		ssd->channel_head[channel].chip_head[chip].next_state=CHIP_IDLE;										
		ssd->channel_head[channel].chip_head[chip].next_state_predict_time=ssd->channel_head[channel].next_state_predict_time+tPROG;
	}
	else
	{
		return NULL;
...
struct ssd_info *un_greed_interleave_copyback(struct ssd_info *ssd, unsigned int channel, unsigned int chip, unsigned int die, struct sub_request *sub1, struct sub_request *sub2)
{
	...
	//rui alter 根据不同的所在页计算不同的介质读写时间
	int sub1_layer = sub1->location->page / (ssd->parameter->page_block / TOTAL_LAYER);
	int sub1_tR = ssd->parameter->time_characteristics.tR * (1 + LAYER_SPPED * sub1_layer / TOTAL_LAYER);
	int sub1_tPROG = ssd->parameter->time_characteristics.tPROG * (1 + LAYER_SPPED * sub1_layer / TOTAL_LAYER);
	int sub2_layer = sub1->location->page / (ssd->parameter->page_block / TOTAL_LAYER);
	int sub2_tR = ssd->parameter->time_characteristics.tR * (1 + LAYER_SPPED * sub2_layer / TOTAL_LAYER);
	int sub2_tPROG = ssd->parameter->time_characteristics.tPROG * (1 + LAYER_SPPED * sub1_layer / TOTAL_LAYER);
	...
	if ((old_ppn1 % 2 == ppn1 % 2) && (old_ppn2 % 2 == ppn2 % 2))
	{
		...
		sub1->next_state_predict_time = ssd->current_time + 14 * ssd->parameter->time_characteristics.tWC + sub1_tR + (sub1->size*ssd->parameter->subpage_capacity)*ssd->parameter->time_characteristics.tWC;
		...
		sub2->next_state_predict_time = sub2->current_time + 14 * ssd->parameter->time_characteristics.tWC + sub2_tR + (sub2->size*ssd->parameter->subpage_capacity)*ssd->parameter->time_characteristics.tWC;
		...
		ssd->channel_head[channel].chip_head[chip].current_time = ssd->current_time;
		ssd->channel_head[channel].chip_head[chip].next_state = CHIP_IDLE;
		ssd->channel_head[channel].chip_head[chip].next_state_predict_time = ssd->channel_head[channel].next_state_predict_time + sub2_tPROG;
	...
	else if ((old_ppn1 % 2 != ppn1 % 2) && (old_ppn2 % 2 == ppn2 % 2))
	{
		...
		sub2->next_state = SR_COMPLETE;
		sub2->next_state_predict_time = ssd->current_time + 14 * ssd->parameter->time_characteristics.tWC + sub2_tR + (sub2->size*ssd->parameter->subpage_capacity)*ssd->parameter->time_characteristics.tWC;
		sub2->complete_time = sub2->next_state_predict_time;
	...
	else
	{
		...
		sub1->current_time = ssd->current_time;
		sub1->next_state = SR_COMPLETE;
		sub1->next_state_predict_time = ssd->current_time + 14 * ssd->parameter->time_characteristics.tWC + sub1_tR + 2 * (ssd->parameter->subpage_page*ssd->parameter->subpage_capacity)*ssd->parameter->time_characteristics.tWC;
		sub1->complete_time = sub1->next_state_predict_time;
		...
		ssd->channel_head[channel].chip_head[chip].current_time = ssd->current_time;
		ssd->channel_head[channel].chip_head[chip].next_state = CHIP_IDLE;
		ssd->channel_head[channel].chip_head[chip].next_state_predict_time = ssd->channel_head[channel].next_state_predict_time + sub1_tPROG;
...
struct ssd_info *un_greed_copyback(struct ssd_info *ssd, unsigned int channel, unsigned int chip, unsigned int die, struct sub_request *sub1)
{
	...
	//rui alter 根据不同的所在页计算不同的介质读写时间
	int layer = sub1->location->page / (ssd->parameter->page_block / TOTAL_LAYER);
	int tR = ssd->parameter->time_characteristics.tR * (1 + LAYER_SPPED * layer / TOTAL_LAYER);
	int tPROG = ssd->parameter->time_characteristics.tPROG * (1 + LAYER_SPPED * layer / TOTAL_LAYER);
	if (old_ppn % 2 == ppn % 2)
	{
		...
		sub1->current_time = ssd->current_time;
		sub1->next_state = SR_COMPLETE;
		sub1->next_state_predict_time = ssd->current_time + 14 * ssd->parameter->time_characteristics.tWC + tR + (sub1->size*ssd->parameter->subpage_capacity)*ssd->parameter->time_characteristics.tWC;
		sub1->complete_time = sub1->next_state_predict_time;
	...
	else
	{
		...
		sub1->current_time = ssd->current_time;
		sub1->next_state = SR_COMPLETE;
		sub1->next_state_predict_time = ssd->current_time + 14 * ssd->parameter->time_characteristics.tWC + tR + 2 * (ssd->parameter->subpage_page*ssd->parameter->subpage_capacity)*ssd->parameter->time_characteristics.tWC;
		sub1->complete_time = sub1->next_state_predict_time;
	...
	if (command == NORMAL)
	{
		...
		case SR_R_READ:
		{
			...
			******************************************************************************************************/
			...
			sub->next_state_predict_time = ssd->current_time + tR;
			...
			ssd->channel_head[location->channel].chip_head[location->chip].next_state_predict_time = ssd->current_time + tR;
		...
		case SR_W_TRANSFER:
		{
			...
			*******************************************************************************************************/
			...
			ssd->channel_head[location->channel].chip_head[location->chip].current_time = ssd->current_time;
			ssd->channel_head[location->channel].chip_head[location->chip].next_state = CHIP_IDLE;
			ssd->channel_head[location->channel].chip_head[location->chip].next_state_predict_time = time + tPROG;
